...
int uid_action()
{
	...
	*(p++)=0;
	...
	return(perform_action_perso(sn_str));
}
int uid_write()
{
    int  	tmp;
...
int perform_action_perso(char * act_str)
{
    char str[255];  
									// look for match to action string
    ...
    if (find_config_param(act_str,str,sizeof(str))>0)
    {
		printf("Utilisateur reconnu\n");
    }
     else
        p_printf(YELLOW, "Utilisateur non reconnu\n");
     return(2);
}
